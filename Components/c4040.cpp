...
// Last update Wed Mar  1 19:23:27 2017 Arnaud WURMEL
//

#include <iostream>
#include "Errors.hpp"
#include "c4040.hpp"
# define RESET_VALUE 17
...
nts::Tristate	nts::c4040::Compute(size_t pin_num_this)
{
  ...
  if ((pin_num_this >= 1 && pin_num_this <= 7) || (pin_num_this >= 12 && pin_num_this <= 15) || pin_num_this == 9)
      ...
      if (reset)
	{
      ...
      if (_pins.find(pin_num_this) != _pins.end())
	{
	  return (outputs[_pins[pin_num_this]] == 1 ? nts::Tristate::TRUE : nts::Tristate::FALSE);
	}
    }
